...
const Header = () => {
  ...
  return (
    <div className="fixed w-full z-50 flex justify-center px-4 pt-6 pointer-events-none">
      <nav className={`
        pointer-events-auto
      ...
      `}>
        {/* Logo Section - Removed "Harshini" text */}
        <div className="flex items-center">
          <a href="#" className="w-9 h-9 lg:w-11 lg:h-11 rounded-full bg-teal-600 flex items-center justify-center text-white font-black text-sm lg:text-lg shadow-lg shadow-teal-500/20 hover:scale-110 transition-transform">
            H
          </a>
        </div>
        
        {/* Desktop Navigation Links - Redundant Contact Link Removed */}
          ...
          <div className="flex items-center lg:space-x-1">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                className={`
                  relative px-3 lg:px-4 py-2 rounded-full text-[0.765rem] lg:text-[0.8925rem] font-black uppercase tracking-[0.15em] transition-all duration-300
        ...
        {/* Action Button - Main Contact Entry Point */}
        <div className="hidden md:flex items-center">
           <a href="#contact" className="ml-2 lg:ml-6 px-6 lg:px-8 py-2.5 bg-[#111827] text-white text-[0.7rem] lg:text-[0.8rem] font-black uppercase tracking-[0.2em] rounded-full hover:bg-slate-800 transition-all shadow-xl hover:-translate-y-0.5 active:translate-y-0">
            Contact
          </a>
        </div>
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-[2.5rem] border border-slate-100 shadow-2xl p-6 space-y-3">
          {navLinks.map((link) => (
            <a
              ...
              className={`
                block px-8 py-4 rounded-3xl text-sm font-black uppercase tracking-widest transition-all
                ${activeSection === link.id ? 'bg-teal-50 text-teal-600' : 'text-slate-600 hover:bg-slate-50'}
              `}
            >
              {link.name}
            </a>
              ...
              <div className="relative z-10 bento-inner">
                ...
                <div className="mt-12 flex flex-wrap items-center gap-8">
                  <div className="flex -space-x-4">
                    {[1,2,3].map(i => (
                      <div key={i} className="w-14 h-14 rounded-2xl border-4 border-slate-900 bg-slate-800 flex items-center justify-center transition-all hover:scale-110 hover:z-10 group/icon">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
        {TECHNICAL_SKILLS.map((category, idx) => (
          <div key={idx} className="bento-card glow-effect bg-slate-50 p-10 rounded-[2.5rem] border border-slate-100 hover:border-teal-200 hover:bg-white transition-all group">
      ...
      <div className="bento-card bg-slate-900 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl glow-effect">
        <div className="bento-inner relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {SOFT_SKILLS.map((skill, idx) => (
              <div key={idx} className="flex items-center gap-4 bg-white/5 p-5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all hover:translate-x-1">
  ...
  <section id="experience" className="py-24 bg-slate-50">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="space-y-12">
        {EXPERIENCE.map((exp) => (
          <div key={exp.id} className="bento-card glow-effect relative bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm transition-all group">
              ...
              <div className="md:col-span-9 border-l-4 border-slate-100 pl-10">
                ...
                <ul className="space-y-5">
                  {exp.description.map((point, i) => (
                    <li key={i} className="flex items-start gap-4 text-slate-600 leading-relaxed text-lg transition-transform hover:translate-x-1">
                      <div className="mt-2 shrink-0">
                        <div className="w-5 h-5 rounded-full bg-teal-100 flex items-center justify-center group-hover:bg-teal-500 transition-colors">
                          <ChevronRight size={14} className="text-teal-600 group-hover:text-white" />
                        </div>
                      </div>
  ...
  <section id="education" className="py-24 bg-white">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {EDUCATION.map((edu) => (
          <div key={edu.id} className="bento-card glow-effect bg-slate-50 p-10 rounded-[2.5rem] border border-slate-100 hover:border-teal-200 transition-all group">
            <div className="bento-inner">
              <div className="w-16 h-16 bg-white text-teal-600 rounded-2xl flex items-center justify-center mb-8 shadow-sm group-hover:gradient-bg group-hover:text-white transition-all duration-500">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {CERTIFICATIONS.map((cert) => (
          <div key={cert.id} className="bento-card glow-effect group relative overflow-hidden rounded-[3rem] bg-white border border-slate-200 p-2 shadow-sm transition-all">
            <div className="bento-inner">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <a 
          ...
          className="bento-card glow-effect p-12 bg-white/5 backdrop-blur-xl rounded-[3rem] border border-white/10 hover:bg-white/10 transition-all group flex flex-col items-center"
        >
          <div className="bento-inner flex flex-col items-center">
            <div className="w-20 h-20 gradient-bg rounded-3xl flex items-center justify-center mb-8 group-hover:scale-110 shadow-2xl shadow-teal-500/20 transition-transform duration-500">
              <Mail size={36} />
            </div>
        ...
        >
          <div className="bento-inner flex flex-col items-center">
            <div className="w-20 h-20 bg-blue-600 rounded-3xl flex items-center justify-center mb-8 group-hover:scale-110 shadow-2xl shadow-blue-500/20 transition-transform duration-500">
              <Linkedin size={36} />
            </div>
        ...
        <div className="bento-card glow-effect p-12 bg-white/5 backdrop-blur-xl rounded-[3rem] border border-white/10 hover:bg-white/10 transition-all group flex flex-col items-center">
          <div className="bento-inner flex flex-col items-center">
            <div className="w-20 h-20 gradient-bg rounded-3xl flex items-center justify-center mb-8 group-hover:scale-110 shadow-2xl shadow-teal-500/20 transition-transform duration-500">
              <MapPin size={36} />
            </div>
